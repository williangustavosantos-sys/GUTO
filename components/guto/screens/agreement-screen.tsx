...
const commitmentTexts: Record<string, { title: string; subtitle: string; instruction: string; complete: string }> = {
  "pt-BR": { 
    title: "Sem volta.", 
...
export function AgreementScreen({ userName, language, onComplete }: AgreementScreenProps) {
  const [holdProgress, setHoldProgress] = useState(0)
  const [isHolding, setIsHolding] = useState(false)
  const [isComplete, setIsComplete] = useState(false)
  const intervalRef = useRef<NodeJS.Timeout | null>(null)
  const commitment = commitmentTexts[language || "pt-BR"]
    ...
    >
      ...
      >
        ...
        >
          ...
          <h2 
            className="text-4xl font-light italic tracking-wide"
            style={{
        ...
        >
          ...
          <motion.div
            ...
            style={{
              background: `conic-gradient(from 0deg, oklch(0.70 0.18 200 / ${holdProgress / 100}) ${holdProgress * 3.6}deg, transparent ${holdProgress * 3.6}deg)`,
            }}
            animate={{
          ...
          {/* Progress ring */}
          <svg 
            className="absolute inset-0 -m-4 w-[calc(100%+32px)] h-[calc(100%+32px)]" 
            viewBox="0 0 120 120"
          >
            <circle
              cx="60"
              ...
              r="54"
              fill="none"
              stroke="oklch(0.70 0.18 200 / 0.2)"
              strokeWidth="4"
            />
            <motion.circle
              cx="60"
              ...
              style={{
                filter: "drop-shadow(0 0 8px oklch(0.70 0.18 200 / 0.6))"
              }}
            />
          </svg>
          ...
          <motion.button
            ...
            onTouchEnd={endHold}
            className="relative w-24 h-24 rounded-full glass-strong flex items-center justify-center transition-all"
            animate={{
